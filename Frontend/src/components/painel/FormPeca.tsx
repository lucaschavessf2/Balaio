...
import { atualizarPeca, criarPeca, type DadosPeca, type FotoParaEnviar } from '@/services/api/pecas.servico'
import type { Disponibilidade, Peca, Tecnica } from '@/types/dominio'
...
const LIMITE_HISTORIA = 4000
const PRAZO_PADRAO_DIAS = 15
const SEPARADOR_PARAGRAFOS = /\n\s*\n/

type Situacao = DadosPeca['situacao']
...
function fotosDaPeca(peca?: Peca): FotoSelecionada[] {
  if (!peca) return []
  if (peca.fotos?.length) return peca.fotos.map(({ id, nome, url }) => ({ id, nome, url }))
  return [{ id: `${peca.slug}-capa`, nome: peca.nome, url: peca.imagem }]
}

function paragrafosDaHistoria(texto: string): string[] {
  return texto.split(SEPARADOR_PARAGRAFOS).map((trecho) => trecho.trim()).filter(Boolean)
}

function prazoInformado(valor: FormDataEntryValue | null): number | null {
  return validarPrazoDias(String(valor ?? '')) ? null : Number(valor)
}

export default function FormPeca({ peca, modelo }: Props) {
  const roteador = useRouter()
  const base = peca ?? modelo
  const editando = Boolean(peca)
  const publicada = editando && (peca?.situacao ?? 'publicada') === 'publicada'
  const [salvando, definirSalvando] = useState(false)
  const { tecnicas, territorios, categorias, tipos, carregando } = useReferencias()
  const [disponibilidade, definirDisponibilidade] = useState<Disponibilidade>(base?.disponibilidade ?? 'disponivel')
  const [preco, definirPreco] = useState(() => base?.preco ? emReais(base.preco) : '')
  const [erros, definirErros] = useState<Record<string, string>>({})
  const [fotosSelecionadas, definirFotosSelecionadas] = useState<FotoSelecionada[]>(() => fotosDaPeca(peca))
  const formulario = useRef<HTMLFormElement>(null)
  const entradaFotos = useRef<HTMLInputElement>(null)
  const fotosAtuais = useRef<FotoSelecionada[]>(fotosSelecionadas)
  ...
  useEffect(() => {
    ...
    function impedirQueSoltarForaAbraAImagem(evento: globalThis.DragEvent) {
      if (Array.from(evento.dataTransfer?.types ?? []).includes('Files')) {
        evento.preventDefault()
      }
    }
    window.addEventListener('dragover', impedirQueSoltarForaAbraAImagem)
    window.addEventListener('drop', impedirQueSoltarForaAbraAImagem)
    return () => {
      window.removeEventListener('dragover', impedirQueSoltarForaAbraAImagem)
      window.removeEventListener('drop', impedirQueSoltarForaAbraAImagem)
      urls.forEach((url) => URL.revokeObjectURL(url))
      urls.clear()
    }
  }, [])

  const titulo = editando ? 'Editar peça' : 'Cadastrar uma peça'
  const migalhas = (
    <Migalhas
      trilha={[
        { texto: 'Painel do artesão', href: '/dashboard' },
        { texto: 'Minhas peças', href: '/dashboard/pieces' },
        { texto: editando ? 'Editar peça' : 'Nova peça' },
      ]}
    />
  )
  ...
  function validar(situacao: Situacao): boolean {
  ...
  function liberarMiniatura(url: string) {
    if (!urlsLocais.current.has(url)) return
  ...
  function adicionarFotos(arquivos: File[]) {
    ...
    if (fotosAtuais.current.length + arquivos.length > MAX_FOTOS) {
      ...
      const invalido = arquivos.find((arquivo) => !FORMATOS_FOTO.includes(arquivo.type))
      ...
      if (invalido) erro = `O arquivo "${invalido.name}" não é JPG, PNG ou WebP.`
  ...
  function mensagemDeSucesso(situacao: Situacao): string {
    if (situacao === 'rascunho') return publicada ? 'Peça retirada da loja e salva como rascunho' : 'Rascunho salvo'
    return publicada ? 'Alterações salvas' : 'Peça publicada na loja'
  }

  async function salvar(situacao: Situacao) {
    if (envioEmAndamento.current || !validar(situacao)) return
    const dados = new FormData(formulario.current!)
    envioEmAndamento.current = true
    definirSalvando(true)
    try {
      const fotos: FotoParaEnviar[] = await Promise.all(fotosAtuais.current.map(async (foto) => ({
        id: foto.id,
        nome: foto.nome,
        url: foto.arquivo ? await lerImagem(foto.arquivo) : foto.url,
      })))
      const precoInformado = numeroDoPreco(String(dados.get('preco') ?? ''))
      const dadosPeca: DadosPeca = {
        nome: String(dados.get('nome')).trim(),
        tecnica: String(dados.get('tecnica')) as Tecnica,
        territorio: String(dados.get('territorio')),
        categoria: String(dados.get('categoria')),
        tipo: String(dados.get('tipo')) as TipoPeca,
        historia: paragrafosDaHistoria(String(dados.get('historia') ?? '')),
        preco: Number.isFinite(precoInformado) && precoInformado > 0 ? precoInformado : 0,
        disponibilidade,
        prazoProducaoDias: disponibilidade === 'encomenda' ? prazoInformado(dados.get('prazo')) : null,
        situacao,
        fotos,
      }
      const resposta = peca ? await atualizarPeca(peca.slug, dadosPeca) : await criarPeca(dadosPeca)
      if (resposta.erro) {
        avisar.erro('Não foi possível salvar', resposta.erro.mensagem)
        return
      }
      avisar.sucesso(mensagemDeSucesso(situacao))
      roteador.push('/dashboard/pieces')
      roteador.refresh()
    } catch {
      ...
      <div className="duas-colunas">
        <form ref={formulario} onSubmit={publicar} noValidate>
          <section className="cartao abaixo-5">
            ...
            <Campo rotulo="Nome da peça" ajuda="Como você chamaria essa peça na feira." erro={erros['peca-nome']} id="peca-nome">
              <input id="peca-nome" name="nome" maxLength={LIMITE_NOME} placeholder="Ex.: Leão Imperial de Tracunhaém" defaultValue={peca?.nome} />
            </Campo>

            <div className="grade-dois">
              <Campo rotulo="Técnica" id="peca-tecnica">
                <select id="peca-tecnica" name="tecnica" defaultValue={base?.tecnica ?? tecnicas[0]}>
                  {tecnicas.map((t) => (
              ...
              <Campo rotulo="Tipo de peça" ajuda="É por aqui que o comprador filtra na loja." id="peca-tipo">
                <select id="peca-tipo" name="tipo" defaultValue={base?.tipo ?? tipos[0]}>
                  {tipos.map((t) => (
                    <option key={t}>{t}</option>
                  ))}
                </select>
            ...
            <div className="grade-dois">
              <Campo rotulo="Categoria" id="peca-categoria">
                <select id="peca-categoria" name="categoria" defaultValue={base?.categoria ?? categorias[0]}>
                  {categorias.map((c) => (
              ...
              <Campo rotulo="Território de origem" id="peca-territorio">
                <select id="peca-territorio" name="territorio" defaultValue={base?.territorio ?? territorios[0]}>
                  {territorios.map((t) => (
                    <option key={t}>{t}</option>
                  ))}
                </select>
            ...
            <Campo
              ...
              ajuda="Conte de onde vem, como é feita, quanto tempo leva. Deixe uma linha em branco entre parágrafos."
              id="peca-historia"
            >
              <textarea
                id="peca-historia"
                name="historia"
                maxLength={LIMITE_HISTORIA}
                placeholder="Cada linha da juba é esculpida uma a uma, num ritual de paciência que dura dias..."
                defaultValue={base?.historia.join('\n\n')}
              />
            </Campo>
          </section>
          ...
          <section className="cartao abaixo-5">
            ...
            >
              ...
              <p id="ajuda-fotos" className="campo-ajuda">
                ...
                Até {MAX_FOTOS} fotos em JPG, PNG ou WebP, de {MAX_TAMANHO_ARQUIVO_MB} MB cada.
          ...
          <section className="cartao">
            ...
            {disponibilidade === 'encomenda' && (
              <Campo
                ...
                id="peca-prazo"
              >
                <input id="peca-prazo" name="prazo" type="number" defaultValue={base?.prazoProducaoDias ?? PRAZO_PADRAO_DIAS} min={1} max={120} />
              </Campo>
            )}
            ...
            <div className="acoes-linha acima-2">
              <button disabled={salvando} type="submit" className="botao botao-primario">
                {publicada ? 'Salvar alterações' : 'Publicar peça'}
              </button>
              <button type="button" className="botao botao-fantasma" disabled={salvando} onClick={() => void salvar('rascunho')}>
                {publicada ? 'Tirar da loja e salvar como rascunho' : 'Salvar rascunho'}
              </button>
            </div>
          </section>
        </form>
        ...
        <aside>
          <ApoioPrecificacao
            ...
            aoUsarPreco={(valor) => {
              definirPreco(emReais(valor))
              definirErros((atuais) => {
                const proximos = { ...atuais }
                delete proximos['peca-preco']
                return proximos
              })
            }}
          />
          ...
          <p className="aviso">
            <IconeAviso />
            <span>
              Sua peça entra na loja <strong>assim que você publicar</strong>. O selo de artesão verificado é
              concedido ao ateliê pela curadoria e não impede suas vendas.
            </span>
          </p>
        </aside>
      </div>
